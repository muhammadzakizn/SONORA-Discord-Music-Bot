...
# ==================== AUDIO STREAMING FROM FTP ====================

@app.route('/api/audio/stream/<artist>/<title>')
def api_audio_stream(artist: str, title: str):
    """
    Stream audio from FTP cache via HTTP.
    
    This allows the bot to stream directly via URL instead of downloading.
    URL format: /api/audio/stream/{artist}/{title}
    ...
    try:
        ...
        if not ftp_cache.is_enabled:
            return jsonify({"error": "FTP cache not enabled"}), 503
        
        # Create a unique filename for local cache
        file_hash = hashlib.md5(f"{artist}_{title}".encode()).hexdigest()[:12]
        local_path = Settings.DOWNLOADS_DIR / f"stream_cache_{file_hash}.opus"
        
        # Check if already in local cache
        if not local_path.exists():
            # Download from FTP (synchronously for Flask)
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            
            try:
                # Check if exists in FTP
                exists = loop.run_until_complete(ftp_cache.exists(artist, title))
                if not exists:
                    return jsonify({"error": "Track not found in FTP cache"}), 404
                
                # Download from FTP
                success = loop.run_until_complete(ftp_cache.download(artist, title, local_path))
                if not success:
                    return jsonify({"error": "Failed to download from FTP"}), 500
            finally:
                loop.close()
        ...
        content_type = 'audio/ogg'  # opus is usually in ogg container
        if str(local_path).endswith('.flac'):
            content_type = 'audio/flac'
        elif str(local_path).endswith('.mp3'):
            content_type = 'audio/mpeg'
        elif str(local_path).endswith('.m4a'):
        ...
        return Response(
            ...
            headers={
                'Content-Disposition': f'inline; filename="{title}.opus"',
                'Accept-Ranges': 'bytes',
...
@app.route('/api/audio/url/<artist>/<title>')
def api_audio_url(artist: str, title: str):
    """
    Get streaming URL for a track (for bot to use).
    Returns the full HTTP URL that can be passed to FFmpeg.
    """
    try:
        from services.storage.ftp_storage import get_ftp_cache
        ...
        if not ftp_cache.is_enabled:
            return jsonify({"error": "FTP cache not enabled", "url": None}), 503
        ...
        if not exists:
            return jsonify({"error": "Track not found", "url": None}), 404
        
        # Build the streaming URL
        # Use environment variable for base URL if available
        import os
        base_url = os.getenv('AUDIO_API_BASE_URL', 'http://localhost:5000')
        stream_url = f"{base_url}/api/audio/stream/{artist}/{title}"
        ...
        logger.error(f"Audio URL fetch failed: {e}")
