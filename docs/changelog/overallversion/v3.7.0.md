...
## Users & Servers Page Overhaul (2024-12-13)

### URL and Navigation
- Renamed page from `/developer/users` to `/developer/users-servers`
- Updated sidebar: "Users" → "Users & Servers"

### Servers Tab
- Shows ALL servers from `/api/guilds` (not just banned)
- Status indicators: Active (green), Banned (red)
- Filters: All / Active / Banned
- Delete button to remove server from list

### Users Tab
- Shows all users from admin API
- Delete button to remove user from list

### Files Modified
- `web/src/app/developer/layout.tsx` - Updated sidebar nav item
- `web/src/app/developer/users-servers/page.tsx` - Complete rewrite (renamed from users/)
