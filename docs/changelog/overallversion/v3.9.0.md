...
## 🔐 Security Overhaul - Bank-Level Authentication
...
### Configuration
- New environment variable: `SONORA_MASTER_KEY` for encryption key
- Auth database stored in `auth.db` (separate from `bot.db`)
...
2. Set master encryption key in `.env`:
   ...
   ```
   If not set, a random key will be generated and logged on first run.
