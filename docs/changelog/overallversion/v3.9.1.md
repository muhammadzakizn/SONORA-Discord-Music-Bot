...
**New Feature**: Upload custom profile avatar with cropping and compression.

**Capabilities:**
- Upload PNG, JPG, JPEG files (max 25MB)
- Drag-to-move cropping with resizable crop area
- Auto-compress to localStorage-friendly size (~500KB)
- Avatar persists across sessions via localStorage
- Syncs to NavLiquidGlass and top-right profile corner

**Files Modified:**
- `web/src/app/admin/profile/page.tsx` - Complete rewrite with upload/crop modal
- `web/src/contexts/SessionContext.tsx` - Added `customAvatar` state and setter
- `web/src/contexts/SettingsContext.tsx` - Added missing profile translations
