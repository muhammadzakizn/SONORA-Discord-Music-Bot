...
class QueueCommands(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot):
        ...
        self.queues: Dict[int, List[Any]] = {}  # guild_id -> list of TrackInfo
    ...
    def _get_lock(self, guild_id: int) -> asyncio.Lock:
        """Get or create lock for a guild's queue"""
        if guild_id not in self._queue_locks:
            self._queue_locks[guild_id] = asyncio.Lock()
        return self._queue_locks[guild_id]
    
    async def add_to_queue_async(self, guild_id: int, metadata: Any) -> int:
        """
        Thread-safe add track to queue (async version)
    ...
    def add_to_queue(self, guild_id: int, metadata: Any) -> int:
        """
        Add track to queue (sync version for backward compatibility)
        
        Note: Prefer add_to_queue_async for thread safety
    ...
    async def get_next_async(self, guild_id: int) -> Optional[Any]:
        """
        Thread-safe get next track from queue (async version)
    ...
    def get_next(self, guild_id: int) -> Optional[Any]:
        """
        Get next track from queue (sync version for backward compatibility)
        
        Note: Prefer get_next_async for thread safety
